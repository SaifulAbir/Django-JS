...
def addIndustry(driver: webdriver.Chrome, data):
    time.sleep(5)
    add_industry = driver.find_element_by_class_name('addlink')
    add_industry.click()
    time.sleep(1)
    try:

        if not pd.isna(data['_name']):
            industry_name = driver.find_element_by_name('name')
            industry_name.clear()
            industry_name.send_keys(data['_name'])
            time.sleep(1)
...
def updateIndustry(driver: webdriver.Chrome, data):
    add_industry = driver.find_element_by_class_name('addlink')
    add_industry.click()
    time.sleep(1)
    try:
