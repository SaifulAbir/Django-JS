...
from job.models import Job, Company, JobType, Qualification, Gender, Experience, Industry, Currency, Career_Advice
...
class Career_AdviceTest(TestCase):
    def test_when_everything_is_given_should_pass(self):
        career_advice = Career_Advice(title='Lorem Ipsum is simply dummy text of the printing and typesetting industry.',
    ...
    def test_when_title_is_blank_should_raise_error(self):
        career_advice = Career_Advice(title=' ')
        with self.assertRaises(ValidationError):
            career_advice.full_clean()
    ...
    def test_when_description_is_blank_should_raise_error(self):
        career_advice = Career_Advice(description=' ')
        with self.assertRaises(ValidationError):
            career_advice.full_clean()
    ...
    def test_when_author_is_blank_should_raise_error(self):
        career_advice = Career_Advice(author=' ')
        with self.assertRaises(ValidationError):
            career_advice.full_clean()

    def test_when_author_is_more_than_max_length_should_raise_error(self):
        career_advice = Career_Advice(author='Lorem Ipsum is simply dummy text of the printing and typesetting industry. '
                             'Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, '
                             'when an unknown printer took a galley of type and scrambled it to make a type '
                             'specimen book. It has survived not only five centuries, but also the leap into '
                             'electronic typesetting, remaining essentially unchanged. It was popularised in '
